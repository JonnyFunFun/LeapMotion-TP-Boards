/** Debug? **/
var LEAP_BOARD_DEBUG = true;
var LEAP_BOARD_LOGGED = false;
...
tau.mashups
	...
	var LeapBoard = function() {
		
		this.init = function () {
			console.log("Starting Leap board mashup...");
			/* motion constants */
			this._X_factor = 3.75;
			this._Y_factor = 2.5;
			this._drag_threshold = 300;
			this._double_click_threshold = 800; /* msec */
			this._circle_close_progress = 2;
			/* variables */
			this.debug_window = null;
			...
			this.animations = {
				clickAnimate: 
					{
						'animation': 'pulseDot 0.25s 1',
						'-webkit-animation': 'pulseDot 0.25s 1',
						'-moz-animation': 'pulseDot 0.25s 1'
					},
				doubleClickAnimate:
					{
						'animation': 'pulseDot 0.25s 2',
						'-webkit-animation': 'pulseDot 0.25s 2',						
						'-moz-animation': 'pulseDot 0.25s 2'
					},
				reset:
					{
						'animation': 'none',
			...
			/* debug window setup */
			if (LEAP_BOARD_DEBUG) {
				this.debug_window = $('<div></div>');
				this.debug_window.css({
					"position"			: "absolute", 
					"right"				: 1,
					"bottom"			: 1,
					"width"				: 200,
				...
				$('body').append(this.debug_window);
			}
			/* animation */
			$('head').append('<style>@-moz-keyframes pulseDot{0%{-moz-transform:scale(0);opacity:.0}25%{-moz-transform:scale(0);opacity:.1}50%{-moz-transform:scale(0.1);opacity:.3}75%{-moz-transform:scale(0.5);opacity:.5}100%{-moz-transform:scale(1);opacity:.0}}@-webkit-keyframes pulseDot{0%{-webkit-transform:scale(0.1);opacity:0}25%{-webkit-transform:scale(0.2);opacity:.1}50%{-webkit-transform:scale(0.5);opacity:.3}75%{-webkit-transform:scale(0.8);opacity:.5}100%{-webkit-transform:scale(1);opacity:0}}@keyframes pulseDot{0%{transform:scale(0);opacity:.0}25%{transform:scale(0);opacity:.1}50%{transform:scale(0.1);opacity:.3}75%{transform:scale(0.5);opacity:.5}100%{transform:scale(1);opacity:.0}}</style>');
			/* bind to Leap's loop */
			var self = this;
			Leap.loop({enableGestures: true}, function(frame) {
				if (self.debug_window)
					self.debug_info(frame);
				self.loop(frame);
			});
		}
		...
		this.loop = function(frame) {
			if (frame.pointables.length >= 1) {
				...
				this.pointer.show();
				/* determine X */
				var x_midpoint = Math.ceil($(window).width() / 2);
				var x_pos = x_midpoint + (position[0].toFixed(1) * this._X_factor);
				/* determine Y */
				var y_pos = (position[1].toFixed(1) - 100) * this._Y_factor;
				this.pointer.css({'bottom': y_pos, 'left': x_pos});
				this.pointer.css(this.animations.reset);
				/* check for clicks */
				if ((frame.gestures) && (frame.gestures.length > 0)) {
					var self = this;
					$.each(frame.gestures, function(idx, gesture) {
						if (gesture.type == "keyTap") {
							/* click! */
							if ((self.last_click) && ((new Date().getTime() - self.last_click) < self._double_click_threshold)) {
								/* double click! */
								self.pointer.css('background-color', '#0f0');
								self.pointer.css(self.animations.doubleClickAnimate);
								self.doubleClick();
							} else {		
								self.last_click = new Date().getTime();
								self.pointer.css('background-color', '#f00');
								self.pointer.css(self.animations.clickAnimate);
								self.click();
							}
						} else if (gesture.type == "screenTap") {
							/* taps */
							self.mouseDown();
							self.tapDepth = gesture.position[2];
							...
							if (gesture.direction[0] > 0.5) {
								...
								if ($('div.close:visible').length != 0)
									$('div.close:visible').trigger('click');
							}
				...
				/* reset up */
				if ((this.last_click) && ((new Date().getTime() - this.last_click) > this._drag_threshold)) {
					this.pointer.css('background-color', '#0f0');
					this.pointer.css(this.animations.reset);
					this.last_click = null;
				}
			} else {
				/* no fingers, hide overlay */
				this.pointer.hide();
				this.last_click = null;
				this.pointer.css('background-color', '#0f0');
				this.pointer.css(this.animations.reset);
		...
		this.drag = function() {
			...
			$(window).simulate('mousemove', {clientX: this.pointer.offset().left, clientY: this.pointer.offset().top});
		}
		
		this.mouseDown = function() {
			console.log("Mouse down");
			this.elementUnderPointer().simulate('mousedown', {clientX: this.pointer.offset().left, clientY: this.pointer.offset().top});
		}
		
		this.mouseUp = function() {
			console.log("Mouse up");
			this.elementUnderPointer().simulate('mouseup', {clientX: this.pointer.offset().left, clientY: this.pointer.offset().top});
		}
		
		this.doubleClick = function() {
			console.log("Double Click!");
			this.elementUnderPointer().dblclick();
		}
		
		this.click = function() {
			console.log("Click!");
			this.elementUnderPointer().trigger('click');
		}
		
		this.elementUnderPointer = function() {
			var el = document.elementFromPoint(this.pointer.offset().left - 1, this.pointer.offset().top - 1);
			el = $(el);
			if (el.is('a') || el.is('div.close'))
				return el;
			if (el.is('span'))
				return el.parent('a');
			if (el.attr('role') != "card") {
				el = el.parents('div[role="card"]');
			}
			return el;
		}
		
		this.debug_info = function(frame) {
			...
			for (var i = 0; i < frame.pointables.length; i++)
			{
				...
				var position = pointable.tipPosition;
			    frameString += "Pointable ID: " + pointable.id + " @ ("
				                    + position[0].toFixed(1) + ","
				                    + position[1].toFixed(1) + ","
				                    + position[2].toFixed(1) + ")";
			}
			
			this.debug_window.html(frameString);
		}
	};
